...
//        popup.add(fullRezItem);
    ...
    public boolean canDataDoProgressiveResolution() {
        // yes, i know the instanceof checks are a code smell and that the
        // visitor pattern should be used instead. i'd just rather avoid making
        // a local override of DataSource/DataChoice (which probably sounds
        // pretty amusing coming from me)...
        List<DataSource> sources = getDataSources();
        for (DataSource source : sources) {
            if ((source instanceof AddeImageParameterDataSource) || (source instanceof SuomiNPPDataSource) || (source instanceof MultiSpectralDataSource)) {
                return false;
            } else if (source instanceof AddeImageDataSource) {
                // temporarily disable ar/mdr for l3 radar...so we need to do a
                // little digging.
        ...
        // sometimes we don't have any data sources (such as when displaying
        // something generated by hydra's combination control)--however, we *do*
        // have data *choices*!
        ...
        try {
            for (DataChoice choice : choices) {
                if ((choice instanceof ComboDataChoice) || (choice instanceof DataDataChoice)) {
                    return false;
                }
...
//                }
            ...
            logger.error("DataDataChoice#getData() threw an exception (should not be possible)", e);
        }
        return true;
    }
